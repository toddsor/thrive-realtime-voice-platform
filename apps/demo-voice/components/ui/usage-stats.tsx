...
export function UsageStats({ 
  ...
  const fetchUsageData = useCallback(async () => {
    ...
    try {
      ...
      if (sessionId) {
        const response = await fetch(`/api/usage/session/${sessionId}`);
        if (response.ok) {
      ...
      // Fetch quota status
      if (userId) {
        const response = await fetch(`/api/usage/quota/${userId}`);
        if (response.ok) {
      ...
      if (sessionId) {
        const response = await fetch(`/api/usage/session-status/${sessionId}`);
        if (response.ok) {
  ...
  // Auto-refresh every 5 seconds when session is active
  useEffect(() => {
    fetchUsageData();
  ...
  }, [sessionId, userId, isActive, fetchUsageData]);
  ...
  const formatQuotaRemaining = (remaining: number): string => {
  ...
  const getQuotaColor = (percentUsed: number): "default" | "destructive" | "secondary" | "outline" => {
    if (percentUsed >= 90) return 'destructive';
    if (percentUsed >= 70) return 'secondary';
    return 'default';
  };

  // Get session status color
  const getSessionStatusColor = (status: SessionStatus): "default" | "destructive" | "secondary" | "outline" => {
    if (status.gracePeriodActive) return 'destructive';
    if (status.percentComplete >= 0.8) return 'secondary';
    return 'default';
  };
      ...
      {sessionUsage && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Real-time usage statistics for your current session
            </CardDescription>
          </CardHeader>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                <div className="text-2xl font-bold">
                  {formatDuration(sessionUsage.durationMs)}
                </div>
                <div className="text-sm text-gray-600">Duration</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold">
                  {sessionUsage.audioMinutes.toFixed(1)}m
                </div>
                <div className="text-sm text-gray-600">Audio</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold">
                  {sessionUsage.tokensInput + sessionUsage.tokensOutput}
                </div>
                <div className="text-sm text-gray-600">Tokens</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold">
                  {formatCost(sessionUsage.estimatedCost)}
                </div>
      ...
      {sessionStatus && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Current session limits and warnings
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div 
                  ...
                  style={{ width: `${Math.min(sessionStatus.percentComplete * 100, 100)}%` }}
                />
              </div>
            </div>
            
            {sessionStatus.warningsTriggered.length > 0 && (
              <div className="text-sm text-amber-600">
                ⚠️ Warnings triggered at {sessionStatus.warningsTriggered.map(w => `${Math.round(w * 100)}%`).join(', ')}
              </div>
            )}
            
            {sessionStatus.gracePeriodActive && (
              <div className="text-sm text-red-600">
                🚨 Grace period active - session will end soon
              </div>
            )}
            
            {sessionStatus.canExtend && (
      ...
      {quotaStatus && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Your current usage limits and remaining allowances
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div 
                  className={`h-2 rounded-full transition-all duration-300 ${
                    quotaStatus.percentUsed >= 90 ? 'bg-red-500' :
                    quotaStatus.percentUsed >= 70 ? 'bg-yellow-500' : 'bg-green-500'
                  }`}
            ...
            {!quotaStatus.allowed && (
              <div className="text-sm text-red-600">
                ❌ Quota exceeded - cannot start new sessions
              </div>
            )}
            
            <div className="text-xs text-gray-500">
              Resets at {new Date(quotaStatus.resetAt).toLocaleString()}
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {sessionUsage && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Detailed cost analysis for this session
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-green-600">
                  -{formatCost((sessionUsage.tokensCached / 1000) * 0.0005 * 0.5)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Tool Calls:</span>

export class RateLimiter {
    ...
    getRemainingRequests(key, config) {
        const entry = this.entries.get(key);
        if (!entry) {
...
export function getUserId(_request) {
...
export const RATE_LIMITS = {
    SESSION_CREATION: {
        ...
        maxRequests: 10,
        keyGenerator: (req) => `session:${getClientIP(req)}`
    },
    TOOL_CALLS: {
        windowMs: 60 * 1000, // 1 minute
        maxRequests: 30,
        keyGenerator: (req) => `tools:${getClientIP(req)}`
    },
    API_REQUESTS: {

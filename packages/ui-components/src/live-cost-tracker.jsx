...
export function LiveCostTracker({ usageData, isActive = false, className, currentModel = "gpt-realtime", modelPricing = DEFAULT_MODEL_PRICING, }) {
    // Convert usageData to LiveUsage format
    const liveUsage = usageData
        ? {
    ...
    const calculateCostBreakdown = () => {
        const pricing = modelPricing[currentModel] || modelPricing["gpt-realtime-mini"];
    ...
    return (<Card className={className}>
      ...
      <CardContent>
        <div className="space-y-4">
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="border border-gray-200 dark:border-gray-700 px-4 py-2 text-center font-mono">
                      {formatCost((Math.max(0, liveUsage.audioTokensInput - liveUsage.audioTokensCached) / 1000) *
          ...
          {/* Additional Costs */}
          {(liveUsage.toolCalls > 0 || liveUsage.retrievals > 0) && (<div className="mt-4 space-y-2 text-sm">
              <h4 className="font-semibold text-sm">Additional Costs</h4>
              {liveUsage.toolCalls > 0 && (<div className="flex justify-between">
                  <span className="text-muted-foreground">Tool Calls ({liveUsage.toolCalls}):</span>
                  <span>{formatCost(costBreakdown.toolCallCost)}</span>
                </div>)}
              {liveUsage.retrievals > 0 && (<div className="flex justify-between">

...
export interface UsageStore {
  ...
  getUserCostSummary(userId: string, period: 'day' | 'month'): Promise<{
    totalCost: number;
...
export class InMemoryUsageStore implements UsageStore {
  ...
  async getUserUsage(userId: string, period: 'day' | 'month'): Promise<UsageAggregation> {
    ...
    const userSessions = Array.from(this.sessionUsage.values())
      .filter(session => 
  ...
  async getAggregatedMetrics(period: 'hour' | 'day' | 'week' | 'month', startTime: number, endTime: number): Promise<MetricsAggregation> {
    const sessionMetrics = Array.from(this.performanceMetrics.values())
  ...
  async getUserCostSummary(userId: string, period: 'day' | 'month'): Promise<{
    totalCost: number;
    averageCostPerSession: number;
    costTrend: 'increasing' | 'decreasing' | 'stable';
    breakdown: {
    ...
    for (const session of userSessions) {
      ...
      if (costBreakdown) {
        ...
        breakdown.tokens += costBreakdown.inputTokenCost + costBreakdown.outputTokenCost - costBreakdown.cachedTokenDiscount;
  ...
  async exportUsageData(sessionIds: string[], format: 'json' | 'csv'): Promise<string> {
    ...
    if (format === 'json') {
      ...
      // CSV format
      const headers = [
        'sessionId', 'userId', 'startTime', 'endTime', 'durationMs', 'audioMinutes',
        'tokensInput', 'tokensOutput', 'tokensCached', 'toolCalls', 'retrievals',

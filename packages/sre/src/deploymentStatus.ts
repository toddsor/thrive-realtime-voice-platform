...
export class DeploymentStatusTracker {
  ...
  private healthChecks: Array<{
    name: string;
    check: () => Promise<{ status: "pass" | "fail"; message?: string; duration: number }>;
  }> = [];
  ...
  async runHealthChecks(): Promise<DeploymentHealth> {
    ...
    const healthStatus =
      passedChecks === totalChecks ? "healthy" : passedChecks >= totalChecks * 0.8 ? "degraded" : "unhealthy";
  ...
  getDeploymentHealth(): Promise<DeploymentHealth> {
    return this.runHealthChecks();
  }

  // Update deployment info (useful for rollbacks or updates)
  updateDeployment(deployment: Partial<DeploymentInfo>): void {
    if (this.currentDeployment) {
